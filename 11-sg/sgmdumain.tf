...
# Security group for web load balancer service
module "sg_web_alb" {
  source       = "git::https://github.com/naveenrajoli4/ter-sg-module-dev.git?ref=main"
  sg_name      = "web_alb"
  description  = "Created for frontend application load balancer in expense prod"
  location     = var.location
  project_name = var.project_name
  environment  = var.environment
  commn_tags   = var.commn_tags
...
# Accepting traffic from internet to web load balancer seurity group 
...
#APP ALB accept traffic from frontend (frontend-->APP ALB)
resource "aws_security_group_rule" "frontend_appALB" {
  type                     = "ingress"
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
...
# frontend accept traffic from web alb (web alb --> frontend)
resource "aws_security_group_rule" "webALB_frontend" {
  type                     = "ingress"
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
...
# frontend accept traffic from public (public --> frontend)
